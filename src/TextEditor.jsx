...
function App() {
  ...
  const processTextAndSend = async () => {
    ...
    try {
      const response = await fetch("http://localhost:5000/write_array", {
        method: "POST",
        headers: {
  ...
  return (
    <div className="flex justify-center flex-col">
      <textarea
        ...
        className="m-10  p-2 text-white bg-silver-shit border-2 border-blue-700 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-300"
      />
      <br />
      <div className="flex justify-center">
        <button
          onClick={processTextAndSend}
          className="px-6 py-2 bg-silver-shit from-blue-500 to-indigo-500 text-white font-semibold rounded-lg shadow-md hover:from-indigo-500 hover:to-blue-500 focus:outline-none focus:ring-2 focus:ring-indigo-300 transition ease-in-out duration-300"
        >
          Run Code
        </button>
      </div>
